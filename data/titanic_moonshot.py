# titanic_moonshot.py
# End-to-end pipeline for Kaggle Titanic:
# - Feature engineering (kun fra train/test)
# - 5-fold CV
# - Ensemble (HistGradientBoosting + XGBoost + LogisticRegression, soft voting)
# - Pseudo-labeling av sikre testprediksjoner
# - Skriver submission.csv + enkel rapport + lagrer modell
...
assert os.path.exists(TRAIN_PATH), "Mangler data/train.csv"
assert os.path.exists(TEST_PATH),  "Mangler data/test.csv"
...
common_titles = {"Mr","Mrs","Miss","Master","Officer","Rare"}
full["Title"] = full["Title"].where(full["Title"].isin(common_titles), "Rare")

full["FamilySize"] = full["SibSp"].fillna(0) + full["Parch"].fillna(0) + 1
full["IsAlone"] = (full["FamilySize"] == 1).astype(int)

full["Ticket_str"] = full["Ticket"].astype(str)
full["TicketPrefix"] = full["Ticket_str"].str.replace(r"[0-9\.\/ ]", "", regex=True).str.upper()
...
sub = pd.DataFrame({"PassengerId": test["PassengerId"], "Survived": final_pred})
...
joblib.dump(pipe, "models/ensemble_pipeline.pkl")
